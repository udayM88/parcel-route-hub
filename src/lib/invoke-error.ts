...
// User-facing variant: returns just the human message the edge function sent
// (e.g. "Please wait 12s before requesting another OTP"), falling back to a
// provided default. supabase-js sets `data = null` on non-2xx replies, so the
// real reason only lives inside error.context.
...
// Reads a numeric `retry_after` (seconds) from a failed invoke response, if any.
export async function getInvokeRetryAfter(error: any): Promise<number | null> {
  const ctx: Response | undefined = error?.context;
  if (!ctx || typeof ctx.text !== "function") return null;
  try {
    const cloned = typeof ctx.clone === "function" ? ctx.clone() : ctx;
    const j = JSON.parse(await cloned.text());
